...
def test_lake_huron_example():
    """Test loo_future using the Lake Huron dataset from the R vignette."""
    ...
    logger.info(f"Water level range: {df['y'].min():.2f} to {df['y'].max():.2f}")

    with pm.Model(coords={"obs_id": range(len(df))}) as model:
        rho = pm.Normal("rho", mu=0, sigma=0.5, shape=4)
        ...
        logger.info("Sampling from the Lake Huron AR(4) model...")
        idata = pm.sample(
            chains=2,
            draws=500,
            tune=500,
    ...
    L = 80  # Minimum observations required
    M = 4  # 1-step ahead prediction

    logger.info("Running 1-step-ahead Leave-Future-Out cross-validation...")
    loo_future_result = loo_future(wrapper, M=M, L=L, pointwise=True)

    logger.info(f"ELPD from LFO-CV: {loo_future_result['elpd_lfo']:.2f}")
    logger.info(f"Number of refits: {len(loo_future_result['refit_indices'])}")
    logger.info(f"Refit indices: {loo_future_result['refit_indices']}")

    if hasattr(loo_future_result, "lfo_i"):
        min_elpd = loo_future_result.lfo_i.min().item()
        max_elpd = loo_future_result.lfo_i.max().item()
        mean_elpd = loo_future_result.lfo_i.mean().item()
        logger.info(f"Pointwise ELPD range: {min_elpd:.2f} to {max_elpd:.2f}")
        logger.info(f"Pointwise ELPD mean: {mean_elpd:.2f}")

    if hasattr(loo_future_result, "pareto_k"):
        max_k = loo_future_result.pareto_k.max().item()
        logger.info(f"Maximum Pareto k: {max_k:.4f}")
