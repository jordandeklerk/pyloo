...
class PyMCWrapper:
    ...
    def sample_mixture_posterior(
        ...
        **kwargs: Any,
        ...
        r"""Sample from a mixture posterior distribution for improved LOO-CV.

        This method implements Mixture Importance Sampling (MixIS) by modifying the
        model's log probability to include a mixture adjustment term. This adjustment
        helps the posterior better approximate the leave-one-out posteriors in high-dimensional
        models.

        The mixture adjustment adds the log of the sum of exponentials of the negative
        log-likelihoods to the target log probability density:

        .. math::
            p(\theta|y) \propto p(y|\theta)p(\theta) \cdot \exp(\log\sum_i\exp(-\log p(y_i|\theta)))
        ...
        progressbar : bool
            Whether to display a progress bar.
        var_name : str | None
            Name of the observed variable for which to compute the mixture adjustment.
            If None, uses the first observed variable.
        **kwargs : Any
            Additional keyword arguments to pass to `pm.sample()`.
        ...
        -------
        InferenceData
            ArviZ InferenceData object containing the posterior samples from the
            mixture distribution and log likelihood values.

        Raises
        ------
        PyMCWrapperError
            If sampling parameters are invalid, if the variable is not found,
            if the log likelihood computation fails, or if the sampling process fails.
        """
        if draws <= 0:
            raise PyMCWrapperError(f"Number of draws must be positive, got {draws}")
        if chains <= 0:
        ...
        try:
            with self.model:
                ...
                log_lik_list = self.model.logp([observed_rv], sum=False)
                log_lik = log_lik_list[0]

                mixture_adjustment = pt.log(pt.sum(pt.exp(-log_lik)))
                pm.Potential("mixture_adjustment", mixture_adjustment)
            ...
            raise PyMCWrapperError(f"Sampling from mixture posterior failed: {str(e)}")
    ...
    def get_log_likelihood(
        ...
        try:
            ...
            if hasattr(self, "observed_dims") and var_name in self.observed_dims:
                ...
                coords = _get_coords(self, var_name)
                if coords is None:
                    coords = {}
